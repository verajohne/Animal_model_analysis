...
def get_markov_probabilities(trajectory):
	'''
	Given a trajectory, calculate the probability
	of moving to each of the 6 spaces
	'''
...
def create_markov_trajectory(initial_position, time, mp = None):
	'''
	given a 2D matrix of initial position create a trajectory for time
	...
	for ts in range(1,time):
		...
		for i in range(p0.shape[0]):
			...
			#the transformation relative to COM
